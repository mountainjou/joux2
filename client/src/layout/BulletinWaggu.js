...
const  BulletinWaggu = ({ match, getGongsiDetail, bulletin:{ gongsiDetail, loading }}) => {
    ...
    return loading === null ? (<div>로딩중</div>) : (<div>

<div className="container" style={{ width: 800, marginTop: 50 }}>
            {/* 제목 */}
            <div className="border-bottom border-dark">
                <h3 className="font-weight-bold">{gongsiDetail.rname}</h3><br />
                <h6 className="text-right">{moment(gongsiDetail.date).format('L')} / {gongsiDetail.uname}</h6>
            </div>
            {/* 컨텐츠 */}

            <div className="justify-content-center" style={{ marginTop: 50, width: 600 }}>
                <a className="text-wrap text-center">
                {gongsiDetail.report}</a></div>
