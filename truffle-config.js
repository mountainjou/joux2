...
function getLoomProviderWithMnemonic(mnemonicPath, chainId, writeUrl, readUrl) {
  const mnemonic = readFileSync(mnemonicPath, "utf-8")
    .toString()
    .trim();
  const seed = mnemonicToSeedSync(mnemonic);
  const privateKeyUint8ArrayFromSeed = CryptoUtils.generatePrivateKeyFromSeed(
    new Uint8Array(sha256.array(seed))
  );
  const privateKeyB64 = CryptoUtils.Uint8ArrayToB64(
    privateKeyUint8ArrayFromSeed
  );
    ...
    loom_mainnet: {
      provider: function() {
        ...
        if (fs.existsSync(privateKeyPath)) {
          ...
          const loomTruffleProvider = getLoomProviderWithMnemonic(
            mnemonicPath,
            chainId,
            writeUrl,
            readUrl
